    // Используя Visual Studio, создайте проект по шаблону WIN Forms. => WPF
    // Создайте програму, в главном окне которой поместите любой текст.
    // Также, должно быть окно настроек (можно реализовать с помощью TabControl).
    // Пользователь может изменять настройки.
    // При повторном запуске приложения настройки должны оставатьс прежними.
    ...
    // В окне настроек реализуйте следующие опции: цвет фона, цвет текста, размер шрифта, стиль шрифта,
    // а также кнопку «Сохранить».
#define Config
